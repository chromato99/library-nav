...
FindDistance("21,17");//test case 우리가 범위 정해서 찾아야함
...
function test(){//경로 찍어주는 함수
 ...
 for(var j=0; j<str.length; j++){ 
   ...
   if(flag==0)
   {
		...
		table1.rows[temp[1]].cells[temp[0]].innerHTML ="●";
		flag=1;
   }
   else{
		if(pre[0]==temp[0])
		{
			for(var i=0; i<=Math.abs(temp[1]-pre[1]); i++)
			{
				...
				table1.rows[(pre[1]+i)].cells[temp[0]].innerHTML ="●";}
				else{
					//console.log(Math.abs(temp[1]-pre[1]));
					table1.rows[(pre[1]-i)].cells[temp[0]].innerHTML ="●";}
		...
		else
		{
			for(var i=0; i<=Math.abs(temp[0]-pre[0]); i++)
			{
				...
				table1.rows[temp[1]].cells[(pre[0]+i)].innerHTML ="●";
				}
				else{
					//console.log(Math.abs(temp[0]-pre[0]));
					table1.rows[temp[1]].cells[(pre[0]-i)].innerHTML ="●";
