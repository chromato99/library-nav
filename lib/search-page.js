...
exports.search = function(request, response, next) {
    ...
    var data = db.query('SELECT * FROM book', (err, data) => {
        ...
        for(var i=0; i<data.length-1; i++) {
            var word = data[i].book_name.toUpperCase();
            search_word=search_word.toUpperCase();
            var result = editDistance(word,search_word);
            
            if((result == 1)||result == 0) {
              if(!resultList.toUpperCase().includes(word.toUpperCase()))
                {
                resultList.push(word);
                }
            }
           else if(word.includes(search_word))
            {
           ...
           /* var test = db.query('SELECT count(*) FROM book where classification>=0 and classification<=100 ');
            console.log(test);*/
        }
        
        
        const list = getList(resultList);
