...
function getBookData(arr) {
    var list = "<ul>";
    for (var i=0; i < arr.length; i++) {  
        list += `<li class = 'source' id = ${i}>${arr[i].book_name}&nbsp;&nbsp;&nbsp;&nbsp;[${arr[i].author}]<br>${arr[i].classification} ${arr[i].book_symbol}</li>`; 
    }
    list += "</ul>";
    return list;
}
...
exports.info = function(request, response, next) {
    ...
    var data = db.query('SELECT * FROM book WHERE book_name=(SELECT book_name FROM book WHERE registration_num=?) and author=(SELECT author FROM book WHERE registration_num=?)', [registration, registration], (err, data) => {
        var formedTemplate = '';
        if(data.length == 0) {
            var formedTemplate = 'No data!!\nWrong access'
        }
        else {
            formedTemplate = getBookData(data);
        }
